module SuperSTI
  PLUGIN_OPTIONS = [:inherit, :table_name, :autobuild].freeze
  DEFAULT_OPTIONS = { autosave: true, dependent: :destroy }.freeze
  INHERITED_BY_DEFAULT = [:inherit, :class_name].freeze
  ...
  class OptionsHash < HashWithIndifferentAccess
    ...
    def merge_with_rules!(merging_hash, rules = {})
      merging_hash.each_pair do |key, new_value|
        ...
        when true
          self[key] = new_value
        when :default, nil
          self[key] = new_value if INHERITED_BY_DEFAULT.include?(key.to_sym) && old_value.blank?
        when Hash
          self[key] = old_value.inherit_merge!(new_value, rules_key)
        end
      end
    end
    ...
    def respond_to_missing?(mid, include_private = false)
      mname = mid.id2name
      (mname.chomp('=') && mid != :[]=) || self[mname] || super
    end
  end
  ...
  class AssociationHash < OptionsHash
    ...
    def plugin_options
      slice *PLUGIN_OPTIONS
    end

    def reflection_options
      except *PLUGIN_OPTIONS
    end
  end
  ...
  class Options
    ...
    def set_fallback_class_name_for(association_name)
      unless @options_hash[association_name][:class_name]
        model_underscore = "#{@klass.name.underscore.tr('/', '_')}_#{association_name}"
    ...
    def load_inherited_for(association_name)
      if @klass.superclass.respond_to?(:super_sti_config) && (merging_options = @klass.superclass.super_sti_config[association_name])
